...
int sc_main(int argc, char *argv[])
{
    ...
    fm.caption("TFSim");
    clock_group.caption("Clock cycle");
    ...
    jump_to.caption("JUMP TO");
    jump_ncy.caption("JUMP");
    exit.caption("EXIT");
    cycles.range(1, 10000, 1);
    plc["rst"] << table;
    plc["jmp"] << cycles;
    plc["jmp_bt"] << jump_to << jump_ncy;
    ...
    jump_to.enabled(false);
    jump_ncy.enabled(false);
    botao.events().click([&]
    {
        if(fila)
        {
            ...
            cycles.enabled(true);
            jump_to.enabled(true);
            jump_ncy.enabled(true);
    ...
    jump_to.events().click([&]
    {
        int escolha = cycles.to_int();
        /*inputbox ibox(fm,"","JUMP TO CYCLE");
        inputbox::integer sl("Choose the cycle:",escolha,1,100, 1);
        if(ibox.show_modal(sl))
            escolha = sl.value();*/
        if(escolha - contclock < 0)
            show_message("Error", "You can't jump back");
        else{
            int tam = escolha - contclock;
            for(int i = 0; i < tam; i++)
                if(sc_is_running()){
                    contclock ++;
                    sc_start();
    ...
    jump_ncy.events().click([&]
    {
        int escolha = cycles.to_int();
        /*inputbox ibox(fm,"","JUMP n-CYCLE's");
        inputbox::integer sl("Choose n:",escolha2,1,100, 1);
        if(ibox.show_modal(sl))
            escolha2 = sl.value();*/
